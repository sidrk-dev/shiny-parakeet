...
static constexpr std::size_t kMaxPacketBytes = 256;
static constexpr std::size_t kMaxEncodedBytes = kMaxPacketBytes + (kMaxPacketBytes / 254) + 2;
...
static_assert(sizeof(CommandPacket) <= kMaxPacketBytes);
static_assert(sizeof(TelemetryPacket) <= kMaxPacketBytes);
static_assert(sizeof(ConfigPacket) <= kMaxPacketBytes);
static_assert(sizeof(ConfigAckPacket) <= kMaxPacketBytes);
static_assert(sizeof(TelemetryRequestPacket) <= kMaxPacketBytes);
...
inline std::size_t expectedPacketSize(PacketType type)
{
    switch (type) {
...
inline bool validatePacket(const PacketT& packet, PacketType type)
{
    ...
    PacketT copy;
    std::memcpy(&copy, &packet, sizeof(PacketT));
...
inline std::size_t cobsEncode(const uint8_t* input, std::size_t length, uint8_t* output, std::size_t output_capacity)
{
    ...
    *code_ptr = code;
    return static_cast<std::size_t>(output - start);
}

inline std::size_t cobsDecode(const uint8_t* input, std::size_t length, uint8_t* output, std::size_t output_capacity)
{
