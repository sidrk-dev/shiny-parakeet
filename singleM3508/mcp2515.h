...
class MCP2515
{
    ...
    public:
        MCP2515(const uint8_t _CS, const uint32_t _SPI_CLOCK, SPIClass * _SPI = nullptr);
        MCP2515(bool defaultSPI = false);
        void setCS(const uint8_t _CS);
        void setClock(const uint32_t _SPI_CLOCK);
        void setSPI(SPIClass * _SPI);
        ERROR reset(void);
        ERROR setConfigMode();
